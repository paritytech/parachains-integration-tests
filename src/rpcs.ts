...
export const checkRpc = (key: string, rpc: Rpc, providers) => {
  ...
  if (chain === undefined) {
    console.log(
      `\n⛔ ERROR: 'chain' should be present for the following rpc id:`,
      key
    );
    process.exit(1);
  ...
  if (method === undefined || call === undefined) {
    console.log(
      `\n⛔ ERROR: 'pallet' & 'method' should be present for the following rpc id:`,
      key
    );
    process.exit(1);
  }
  ...
  if (args === undefined) {
    console.log(
      `\n⛔ ERROR: 'args' should be present for the following rpc id:`,
      key
    );
    process.exit(1);
  }
